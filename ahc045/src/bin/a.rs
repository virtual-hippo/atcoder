...
impl Info {
    fn new(input: &Input) -> Self {
        ...
        let dist2 = build_dist2_matrix(input, &centers);
        Self { centers, dist2 }
    }
}

/// 長方形の中心点間の距離の2乗を計算する
fn build_dist2_matrix(input: &Input, centers: &Vec<(usize, usize)>) -> Vec<Vec<usize>> {
    let mut dist = vec![vec![0; input.n]; input.n];
    for i in 0..input.n {
        for j in 0..input.n {
            dist[i][j] =
                (centers[i].0 - centers[j].0).pow(2) + (centers[i].1 - centers[j].1).pow(2);
        }
    }
    dist
}
